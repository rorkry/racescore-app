...
agari = row.get("上り3F", "")
    ...
    html = f"""
    <div style='line-height:1.1; font-size:10px; text-align:center;'>
...
st.title(":clipboard: 出馬表フィルタ - シンプル馬柱表示")
...
st.title(":clipboard: 出馬表フィルタ - 出馬表表示")
...
def display_race_table(df, race_label): for idx, row in df.iterrows(): col1, col2, col3 = st.columns([0.3, 2, 12]) with col1: mark = st.selectbox("", 印リスト, key=f"mark_{race_label}{row['馬名']}{idx}", label_visibility="collapsed") st.markdown(f"<div style='font-size:16px; text-align:center;'>{mark}</div>", unsafe_allow_html=True) with col2: st.markdown(f"<div style='text-align:center; font-weight:bold; color:{TEXT_COLOR};'>{row['馬名']}<br><span style='font-size:10px'>{row['性別']}{row['年齢']}・{row['斤量']}kg</span></div>", unsafe_allow_html=True) with col3: html_row = "<table style='width:100%; text-align:center; border-spacing:0'><tr>" for col in [f"{i}走前" for i in range(1, 6)]: html = row[col] if pd.notnull(row[col]) else f"<div style='min-height:100px; color:{TEXT_COLOR};'>ー</div>" html_row += f"<td style='vertical-align:top; min-width:140px'>{html}</td>" html_row += "</tr></table>" st.markdown(html_row, unsafe_allow_html=True)
...
df_entry["課程教師"] = df_entry["所属"].astype(str) + "/" + df_entry["課程教師"].astype(str)
...
df_past5 = pd.DataFrame(result, columns=["馬名"] + [f"{i}走前" for i in range(1, 6)])
...
try["調教師"] = df_entry["所属"].astype(str) + "/" + df_entry["調教師"].astype(str)
df_entry.drop(columns=["所属"], inplace=True)
...
ged["開催地"].astype(str) + df_merged["R"].astype(str) + "R " + df_merged["レース名"].astype(str)
