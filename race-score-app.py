...
agari = row.get("上り3F", "")
    ...
    return f"<div style='min-height:80px;'>Error: {e}</div>"

def display_race_table(df, race_label): for idx, row in df.iterrows(): col1, col2, col3 = st.columns([0.3, 2, 12]) with col1: mark = st.selectbox("", 印リスト, key=f"mark_{race_label}{row['馬名']}{idx}", label_visibility="collapsed") st.markdown(f"<div style='font-size:16px; text-align:center;'>{mark}</div>", unsafe_allow_html=True) with col2: st.markdown(f"<div style='text-align:center; font-weight:bold; color:{TEXT_COLOR};'>{row['馬名']}<br><span style='font-size:10px'>{row['性別']}{row['年齢']}・{row['斤量']}kg</span></div>", unsafe_allow_html=True) # メモ機能は非表示 # if st.toggle("📝", key=f"toggle_{race_label}{row['馬名']}{idx}"): #     memo = memo_data.get(row["馬名"], "") #     new_memo = st.text_area("", memo, key=f"memo_{race_label}{row['馬名']}{idx}") #     memo_data[row["馬名"]] = new_memo with col3: html_row = "<table style='width:100%; text-align:center; border-spacing:0'><tr>" for col in [f"{i}走前" for i in range(1, 6)]: html = row[col] if pd.notnull(row[col]) else f"<div style='min-height:100px; color:{TEXT_COLOR};'>ー</div>" html_row += f"<td style='vertical-align:top; min-width:140px'>{html}</td>" html_row += "</tr></table>" st.markdown(html_row, unsafe_allow_html=True)
...
f"<td style='vertical-align:top; min-width:140px'>{html}</td>" html_row += "</tr></table>" st.markdown(html_row, unsafe_allow_html=True)
...
・{row['斤量']}kg</span></div>", unsafe_allow_html=True) if st.toggle("📝", key=f"toggle_{race_label}{row['馬名']}{idx}"): memo = memo_data.get(row["馬名"], "") new_memo = st.text_area("", memo, key=f"memo_{race_label}{row['馬名']}{idx}") memo_data[row["馬名"]] = new_memo with col3: html_row = "<table style='width:100%; text-align:center; border-spacing:0'><tr>" for col in [f"{i}走前" for i in range(1, 6)]: html = row[col] if pd.notnull(row[col]) else f"<div style='min-height:100px; color:{TEXT_COLOR};'>ー</div>" html_row += f"<td style='vertical-align:top; min-width:140px'>{html}</td>" html_row += "</tr></table>" st.markdown(html_row, unsafe_allow_html=True)
