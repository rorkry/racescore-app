...
CSS: セルの余白・配置を詰めて格子状にする

st.markdown(""" <style> div[data-baseweb="select"] { background-color: white !important; color: black !important; border-radius: 3px; padding: 0 !important; height: 30px; } td { padding-top: 1px !important; padding-bottom: 1px !important; border: 1px solid #ccc; } table { border-collapse: collapse; } </style> """, unsafe_allow_html=True)
...
CSS: レイアウトを細く整えて格子状に表示

st.markdown(""" <style> div[data-baseweb="select"] { background-color: white !important; color: black !important; border-radius: 3px; padding: 0 !important; height: 30px; } td { padding-top: 1px !important; padding-bottom: 1px !important; border: 1px solid #ccc; } table { border-collapse: collapse; } </style> """, unsafe_allow_html=True)

st.title(":clipboard: 出馬表フィルタ - 印・馬柄横並び表示")
...
st.title(":clipboard: 出馬表フィルタ - 印・馬柱横並び表示")
...
def format_past_row(row):
    try:
        ...
        html = f"""
        <div style='line-height:1.1; font-size:10px; text-align:center; color:{TEXT_COLOR};'>
...
def display_race_table(df, race_label):
    for idx, row in df.iterrows():
        ...
        with col1:
            mark = st.selectbox("", 印リスト, key=f"mark_{race_label}{row['馬名']}{idx}", label_visibility="collapsed")
            st.markdown(f"<div style='font-size:16px; text-align:center;'>{mark}</div>", unsafe_allow_html=True)
        with col2:
            st.markdown(
                f"<div style='text-align:center; font-weight:bold; color:{TEXT_COLOR};'>"
                f"{row['馬名']}<br><span style='font-size:10px'>{row['性別']}{row['年齢']}・{row['斤量']}kg</span></div>",
                unsafe_allow_html=True)
        with col3:
...
import streamlit as st import pandas as pd import unicodedata
...
印リスト = ["", "◎", "◎", "○", "▲", "△", "⭐️", "×", "消"] THEME = st.get_option("theme.base") TEXT_COLOR = "black" if THEME == "light" else "white"
...
def display_race_table(df, race_label): for idx, row in df.iterrows(): col1, col2, col3 = st.columns([0.3, 2, 12]) with col1: mark = st.selectbox("", 印リスト, key=f"mark_{race_label}{row['馬名']}{idx}", label_visibility="collapsed") st.markdown(f"<div style='font-size:16px; text-align:center;'>{mark}</div>", unsafe_allow_html=True) with col2: st.markdown( f"<div style='text-align:center; font-weight:bold; color:{TEXT_COLOR};'>" f"{row['馬名']}<br><span style='font-size:10px'>{row['性別']}{row['年齢']}・{row['斤量']}kg</span></div>", unsafe_allow_html=True) with col3: html_row = "<table style='width:100%; text-align:center; border-spacing:0'><tr>" for col in [f"{i}走前" for i in range(1, 6)]: html = row[col] if pd.notnull(row[col]) else f"<div style='min-height:100px; color:{TEXT_COLOR};'>ー</div>" html_row += f"<td style='vertical-align:top; min-width:140px'>{html}</td>" html_row += "</tr></table>" st.markdown(html_row, unsafe_allow_html=True)
...
") new_memo = st.text_area("", memo, key=f"memo_{race_label}{row['馬名']}{idx}") memo_data[row["馬名"]] = new_memo with col3: html_row = "<table style='width:100%; text-align:center; border-spacing:0'><tr>" for col in [f"{i}走前" for i in range(1, 6)]: html = row[col] if pd.notnull(row[col]) else f"<div style='min-height:100px; color:{TEXT_COLOR};'>ー</div>" html_row += f"<td style='vertical-align:top; min-width:140px'>{html}</td>" html_row += "</tr></table>" st.markdown(html_row, unsafe_allow_html=True)
